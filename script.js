...
function mostrarMatrizMensaje() {
    ...
    for (let i = 0; i < valores.length; i += 2) {
        if (i > 0) matriz += ' ';
        matriz += '[' + valores[i];
        if (i + 1 < valores.length) matriz += ', ' + valores[i + 1];
        else matriz += ', 23';
        matriz += ']';
    }
    matriz += ']';
...
function obtenerMatrizInversa(key) {
    ...
    const a = key[0][0], b = key[0][1], c = key[1][0], d = key[1][1];
    const inv = [
...
function generarMapaOriginal(texto) {
    ...
    for (let i = 0; i < texto.length; i++) {
        if (/^[A-Za-z]$/.test(texto[i])) mapaOriginal.push({ tipo: 'L', valor: texto[i] });
        else mapaOriginal.push({ tipo: 'E', valor: texto[i] });
    }
}
